...
- **`taxonomy.yaml`** (tracked in the repo, fetched from
  ...
  curated vocabulary: 542 tags, 18 roots, at most four levels deep, each with an `id`
  (slug), an optional `parent`, a `kind` (technology or topic) and a `desc`. This is
  what becomes Reef's `Subject` tree.
- **`rfc-tags.json`** (a gitignored build product, published to
  `rfc-editor.github.io/rfc-subject-tags/rfc-tags.json`, rebuilt daily by that repo's
  own CI) is the per-RFC output of running their matching engine against
  `taxonomy.yaml`: `{"RFC9000": {"tags": [...], "paths": [...], ...}, ...}`, one entry
  per published RFC. `tags` is the leaf assignment; `paths` is each leaf's full
  ancestor chain. This is what becomes Reef's `SubjectAssignment` rows.
...
|---|---|---|
| `id` | `slug` | Direct. Both a flat, globally unique namespace. |
| `parent` | `parent` | Direct. Single-parent tree, and `MAX_DEPTH = 4` in subjects/models.py already matches "at most four levels deep" — Reef's cap was plainly set from this same source originally (see below). |
...
| *(no equivalent)* | `name` | `taxonomy.yaml` has one text field (`desc`), not two — no separate display name to import. A newly-created subject gets the same mechanical placeholder `import_subjects.py` already uses for one it creates without a curated name (`title_case(slug)`, e.g. `dns-privacy` -> "Dns Privacy"); an existing subject's `name` is never touched by a sync, since there is nothing upstream to justify a specific overwrite value. |
...
| `rfc-tags.json[rfc].tags` (leaf tags) | `SubjectAssignment` (one row per doc per leaf subject) | Direct, and exactly Reef's own model: `subjects/tree.py`'s `rollup()`/`covering_subject_ids()` already derive ancestor coverage from `path` prefixes at read time, so only the leaf needs a row — matching `paths` in the same JSON record would be redundant, importing what Reef already computes. |

So the core vocabulary (slug, parent, description, the four-level cap) maps directly —
...
### The one real gap: renames

`taxonomy.yaml` has no rename record — R17 in its own README says tags "change rarely;
the expected operations are split and rename," and a rename there is just an `id` that
changed with every reference updated alongside it. Reef, by contrast, treats a rename as
first-class: `Subject.save()` leaves the old slug behind as a `SubjectAlias`
automatically (subjects/models.py), specifically so a published link keeps resolving. A
sync can't tell "the tag was renamed" from "the tag was retired and an unrelated one was
added" — both look identical from the outside: an id vanishes, a different one appears.

A slug that drops out of `taxonomy.yaml` gets retired in Reef (below), and the sync
then computes candidate successors for it from document overlap and text similarity
(the "merge suggestions" feature, further down) — evidence a human can act on with one
click, rather than either silently guessing or leaving staff to notice a retirement and
go looking for what replaced it by hand. It's still a suggestion, not an automatic
merge: R17 says renames should be rare, and the score is only ever surfaced when there
is real evidence (document overlap or a close description match) behind it, never
forced onto a retirement that has none.
...
source for: `description` and `parent` are overwritten even where staff hand-edited
them, and it removes what the source no longer has. `name` is the one field this
can't apply to — `taxonomy.yaml` has no field for it, only `desc`, which maps to
`description` (see below), so a synced subject's `name` is never touched once set.
...
precompute button, rather than round-tripping a diff of up to 542 subjects and tens of
...
def diff_vocabulary(taxonomy: dict) -> VocabularyDiff # to_create, to_update, to_retire
def diff_assignments(rfc_tags: dict) -> AssignmentDiff  # to_create, to_delete
...
- no duplicate `id`;
- no catch-all id (`misc`, `other`, `general` — R12 in their own README, and a rule
  Reef's own vocabulary should hold too).

**Computing the vocabulary diff**, against `Subject.all_objects.all()` (not the live
manager — a slug that reappears in `taxonomy.yaml` after being retired should be
recognised and brought back, not created a second time under a slug the unique
constraint would refuse):

- **to_create**: a taxonomy `id` naming no existing Reef subject. Created with
  `name=title_case(slug)`, `description=desc`, `parent` resolved from the taxonomy's
  `parent`.
- **to_update**: an existing subject whose `parent` or `description` (from `desc`)
  differs from the taxonomy. `name` is never part of this diff — there is no taxonomy
  field to compare it against, see the mapping table above. Reported as a field-level
  diff — `old -> new` for each changed field — not just a count, so the result page
  shows what actually moved or reworded rather than making staff go and look. `kind` is
  read and discarded, per the mapping table above.
- **to_retire**: a currently-*live* Reef subject whose slug is absent from the
  taxonomy. `Subject.retire()` already refuses one with live children unless the whole
  subtree goes together, so retirement is applied depth-first — same as the existing
  "retire subtree" admin action.
...
`subjects/tree.py:rollup()` already derives from Reef's own `path` column. Resolved by
slug against the vocabulary as it will be *after* this sync's create/update/retire
step, so a brand new tag's assignments resolve correctly in the same run. A `tags`
...
- `INFO`, one line each, for every subject created, updated or retired — at most 542
  lines;
...
- a new taxonomy tag creates a subject; a changed `desc` or `parent` updates it and is
  reported as a field diff;
...
- a new (subject, doc) pair creates an assignment; a vanished one deletes it; an
  unresolved slug is logged and skipped rather than raised;
