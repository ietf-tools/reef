...
Pink is the IETF self-hosted survey and engagement service, part of the RFC
...
- Ratings, popularity, and subscriptions: APIs consumed by Red (their UI lives
  in Red). Pink owns storage and aggregation, and sends subscription emails.
...
into `pink_api.yaml`, which both the Nuxt client (via openapi-typescript) and
Red consume.

```
Browser -> NGINX :8088 -> Django + DRF :8001 -> PostgreSQL
                       -> Nuxt runner :3000
Red -> /api/pink/... (bearer / anonymous) -> Django + DRF
```
...
The app image builds `FROM ghcr.io/ietf-tools/pink-app-base`. Until CI publishes
it, build it locally first:

```
docker build -f docker/base.Dockerfile -t ghcr.io/ietf-tools/pink-app-base:latest .
```
...
- http://localhost:8088/api/pink/schema/swagger-ui/ - API docs
...
./manage.py spectacular --file pink_api.yaml --validate   # API schema
...
- `pink/` - Django project (settings package, celery, urls)
- `pinkauth/` - custom user and OIDC (login backend plus bearer resource-server auth)
